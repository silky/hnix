...
replaceStrings tfrom tto ts =
  do
    ...
    when (length nsListMatch /= length nsListReplace)
      $  throwError
      $  ErrorCall "builtins.replaceStrings: Arguments `from`&`to` are lists `from` what replace `to` what, so the number of their inhabitanting elements must always match."
      ...
      go remainder processedAccum ctx =
       ...
       where
        ...
        maybePrefixMatches nsListMatch src =
         ...
         where
          textListMatch = map stringIgnoreContext nsListMatch
